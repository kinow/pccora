...
pccora_identification = Struct("pccora_identification",
                               ...
                               ExprAdapter(SLInt16("year"),
                                           ...
                                           decoder=lambda obj, ctx: 1900 + obj if obj >= 20 else 2000 + obj
                                           if obj != -32768 and obj < 1000 else obj),
                               SLInt16("month"),
                               SLInt16("day"),
                               SLInt16("julian_date"),
                               SLInt16("hour"),
                               ...
                               Value("datetime", lambda ctx: None if ctx['year'] == 0 or ctx['month'] == 0 or ctx[
                                   'day'] == 0 else datetime(ctx['year'], ctx['month'], ctx['day'], ctx['hour'],
                                                             ctx['minute'])),
                               Value("launch_time",
                                     lambda ctx: None if ctx['datetime'] is None else ctx['datetime'] + timedelta(
                                         seconds=ctx['time_elapsed']))
                               )
...
pccora_data = Struct("pccora_data",
                     ...
                     ExprAdapter(Bytes("significance_key", 2),
                                 ...
                                 decoder=lambda obj, ctx: [bin(obj[i]) for i in
                                                           range(0, len(obj))] if obj != -32768 else obj
                                 ),
