            ...
            if m:
                ...
                if line.count('|') == 4 and line.count('hPa') == 0:
                    ...
                    if len(std_levels_col) > 0:
                        ...
                        if is_int(hpa):
                            hpa = str(
                                float(hpa))  # to match the key in the dict... due to different units in the two files
                            if hpa not in dates[d]['press_readings']:
                    ...
                    for x in range(1, 4):
                        ...
                        if len(sig_levels_col) > 0:
                            ...
                            if is_int(height):
                                if height not in dates[d]['height_readings']:
...
class SignificantLevel(object):
    ...
    def __repr__(self):
        return "SignificantLevel(press=%s, altitude=%s, temp=%s, rh=%s, fp=%s, speed=%s, direction=%s)" % \
               (self.press, self.altitude, self.temp, self.rh, self.fp, self.speed, self.direction)
...
class SignificantLevelsState1(State):
    ...
    def value(self, line):
        s = None
        self.next_state = None
        if line.strip() == '' and self.title_found:
            self.white_lines += 1
        ...
        if self.headers2_found:
            if line.strip()[0].lower() != 's':
                ...
                if len(values) >= 7:
                    ...
                    if secs not in self.txt_file.significant_levels:
                        self.txt_file.significant_levels[secs] = SignificantLevel(press=press, altitude=height,
                                                                                  temp=temp, rh=rh, fp=fp, speed='',
                                                                                  direction='')
            ...
            try:
                idx = line.index('Significant levels: Temperature/Humidity') + len(
                    'Significant levels: Temperature/Humidity')
                if idx >= 0:
                    self.reset_states()
...
class StandardPressureLevel(object):
    ...
    def __repr__(self):
        return "StandardPressureLevel(press=%s, altitude=%s, temp=%s, rh=%s, fp=%s, ascrate=%s, speed=%s," \
               "direction=%s)" % (self.press, self.altitude, self.temp, self.rh, self.fp, self.ascrate,
            ...
            if line.strip()[0].lower() != 's':
                ...
                if len(values) >= 8:
                    ...
                    if secs not in self.txt_file.standard_pressure_levels:
                        self.txt_file.standard_pressure_levels[secs] = StandardPressureLevel(press=press,
                                                                                             altitude=height, temp=temp,
                                                                                             rh=rh, fp=fp,
                                                                                             ascrate=ascrate, speed='',
                                                                                             direction='')
                    # FIXME: add to element
                    # pprint(minutes)
                    self.next_state = self
...
def write_netcdf_file(data, output_file):
    ...
    # dataset.wind_computing_mode = str(ident['wind_computing_mode']) + \
    #     ', comment: 0 = Remote 1 = Local 2 = Differential'
    ...
    # dataset.exclude_loran_transmitters = str(ident['exclude_loran_transmitters']) + \
    #     ',comment: One bit for each in the chain; and chain 2 transmitted'
    # dataset.phase_integration_time = str(ident['phase_integration_time']) + \
    #     ', comment: 0 = time (s) 1 = Altitude (m/MSI'
